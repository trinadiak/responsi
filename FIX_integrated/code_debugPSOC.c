...
uint16_t debugCalculateCRC(uint8_t *buffer, int length) {
    uint16_t crc = 0xFFFF;
    for (int pos = 0; pos < length; pos++) {
        crc ^= buffer[pos];
        UART_PutString("Byte ");
        UART_PutChar(buffer[pos]);
        UART_PutString(" -> Intermediate CRC: ");
        
        char crcStepBuffer[10];
        sprintf(crcStepBuffer, "%04X", crc);
        UART_PutString(crcStepBuffer);
        UART_PutString("\r\n");
